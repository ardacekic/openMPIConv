...
def plot_speedup_simple_vs_chwise(channel_sizes, kernel_sizes, np_values, ch_kernel_collections):
    ...
    filename = "blas_multiplication_plot_speedup_simple_vs_chwise.png"
...
def plot_speedup_simple_vs_krnwise(channel_sizes, kernel_sizes, np_values, ch_kernel_collections):
    ...
    plt.legend()
    plt.grid(True, linestyle="--", alpha=0.7)
    filename = "blas_multiplication_plot_speedup_simple_vs_krnwise.png"
    plt.savefig(filename)
    print(f"Combined plot saved as {filename}")
    plt.tight_layout()
    plt.show()
...
def plot_speedup_comparison(channel_sizes, kernel_sizes, np_values, ch_kernel_collections):
    ...
    plt.legend()
    plt.grid(True, linestyle="--", alpha=0.7)
    filename = "blas_multiplication_plot_speedup_comparison.png"
    plt.savefig(filename)
    print(f"Combined plot saved as {filename}")
    plt.tight_layout()
    plt.show()
...
def plot_all_execution_times(channel_sizes, kernel_sizes, np_values, ch_kernel_collections):
    ...
    filename = "blas_multiplication.png"
    plt.savefig(filename)
    print(f"Combined plot saved as {filename}")
...
def plot_all_execution_times_nosimple(channel_sizes, kernel_sizes, np_values, ch_kernel_collections):
    ...
    filename = "blas_multiplication_nosimple.png"
    plt.savefig(filename)
    print(f"Combined plot saved as {filename}")
    ...
    for kernel_size in kernel_sizes:
        for np_value in np_values:
            ...
            filenames = [
                ...
                "mpi_convolution_output_BLAS_CHwise_time.txt",
                "mpi_convolution_output_BLAS_KRNLwise_time.txt"
            ]
            ...
            for _ in range(100):
                ...
                if only_once == 1:
                    os.system("gcc -O3 -std=c99 -o conv.o simple_conv_blas.c -I/usr/include/x86_64-linux-gnu/openblas64-pthread -L/usr/lib/x86_64-linux-gnu/openblas64-pthread -lopenblas64 && ./conv.o")
                    os.system("python3 compare_Blas_Simple.py")
                    only_once = 0
